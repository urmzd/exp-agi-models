...
The linear-attention state in the current implementation is a specific instance of the flat key-value store option, where keys and values are projected from vocabulary space through Fourier-basis coefficients, and the state matrix accumulates key-value outer products with exponential decay.
...
These optimizations preserve the shared-memory-bank model: the linear-attention state still mediates all cross-position communication, and the register ops still perform local computation. The v4_weight_shared changes are purely about parameter efficiency, not architectural philosophy.
