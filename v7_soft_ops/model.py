"""
v7_soft_ops: Differentiable register machine with soft addressing and
soft (Gumbel-softmax) op-bank selection. Cross-position mixing uses the
same linear-attention-with-decay as v3/v4.
...
Per step:
  1. READ   — soft-select source registers via learned address weights
             (softmax over V produces a distribution; output is a weighted
             sum of register values).
  2. SELECT — pick an op from a shared N-op bank via Gumbel-softmax. Soft
             during training, converges toward one-hot with temperature
             annealing.
  3. APPLY  — run the (softly) selected op on the read values.
  4. WRITE  — soft-select destination registers; scatter the op output.

Cross-position: decay-weighted linear attention (same core as v3).

Notes:
- Training is documented as unstable (two major loss spikes reported in
  README). The soft op-selection path is the likely source.
- Conceptually similar to Neural Programmer / Neural Turing Machine style
...
class SoftOpsLM(AgiModel):
    """Differentiable register machine with soft addressing + Gumbel op selection."""

    version = "v7_soft_ops"
    architecture = "Soft-op register machine"
    cross_position = "Decay-weighted linear attention"
    within_position = "Soft read + Gumbel op-bank + soft write"
