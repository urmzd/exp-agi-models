"""
v6_banded_fourier: Fourier basis over vocab indices partitioned into three
frequency bands (low / mid / high); each band drives its own V -> C
projection, with gated multiplicative coupling between band outputs.

Hidden state is V-dimensional; input is one-hot; no output projection.

Per step, cross-position:
  - Long-range memory: high-decay linear attention, projections use the
    low-frequency band of the basis.
  - Short-range memory: low-decay linear attention, projections use the
    high-frequency band.
  - Coupling: long-range output gates short-range output multiplicatively.

Per step, within-position:
  - Three parallel band projections (low / mid / high) feed three channel
    streams, recombined through learned scalar weights, with a gating path
    from the low-band stream onto the high-band stream.

Mathematical shape:
  Fourier basis Phi in R^{V x 2K} is partitioned by harmonic index into
  Phi_L, Phi_M, Phi_H. Each band has its own learned (C, *) coeff matrix.
  Coupling: g = sigmoid(h_L @ W + b); h_H <- g * h_H.

Caveat: the frequency framing is structural, not semantic. Vocab ids are
arbitrary BPE outputs, so "low-frequency" over vocab-id ordering has no
linguistic interpretation. Relative to v3, this gives several independent
low-rank V -> C mixers with gated combination, which is a more flexible
parameterization than a single basis at similar parameter count.

Result per README: 824K params, val_loss 5.66, still descending.
"""
...
class BandedFourierLM(AgiModel):
    """Band-partitioned Fourier linattn with gated cross-band coupling."""

    version = "v6_banded_fourier"
    architecture = "Banded Fourier linattn"
    cross_position = "Per-band decay-weighted linattn with gated coupling"
    within_position = "Parallel band projections with gated combination"
