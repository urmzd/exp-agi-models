"""
v13_with_embedding: CONTROL VARIANT. Intentionally breaks the repo's
no-embedding / no-output-projection thesis by prepending a learned
factored embedding Embedding(V, d) -> Linear(d, V) before the register
state.

Why it exists: to measure what the no-embedding constraint costs. Every
other variant starts from a strict one-hot register; this one gives the
model a learned similarity structure so that related tokens start with
similar activations. The body of the model (sparse read/write routing in
vocab space) is identical to v12_vocab_slice.

If v13 substantially outperforms v12 at matched param count, the no-
embedding constraint is the limiting factor; if not, the constraint is
cheap. Do not reuse this variant as a template for new ones — it is the
labeled exception, not a pattern.

Hidden state is V-dimensional (after the embedding projection), but
input is token_id routed through an embedding; output is still the
register state (no output projection after the body).
"""
...
class WithEmbeddingLM(AgiModel):
    """Control variant: adds Embedding(V, d) -> Linear(d, V) before the v12 body.

    Thesis-breaking on purpose. Every other variant keeps the strict
    one-hot / no-output-projection constraint; this one exists to measure
    what the constraint costs. Do not reuse as a template.
    """

    version = "v13_with_embedding"
    architecture = "Control — breaks no-embedding thesis"
    cross_position = "Decay-weighted linattn in k-slice"
    within_position = "Factored embedding then MLP in k-slice"
