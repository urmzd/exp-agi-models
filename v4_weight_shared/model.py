"""
v4_weight_shared: Parameter-reduced variant of v3_fourier_linattn, targeting
~101K params (down from 329K) via five weight-sharing and factorization
tricks.

Same core as v3 (Fourier-parameterized linear attention with causal decay),
with:

1. Channel mix factored into diagonal + low-rank (16K -> 2.2K per step).
2. Q and K projections shared across all steps (paid once, used many).
3. Per-head decay (H=4) instead of a single scalar, giving multiple
   effective timescales at a few extra params per step.
4. Five unique step modules invoked twice each, with small per-invocation
   override scalars (decay / out_scale / mem_scale / op_scale).
5. Q/K normalization for training stability.

Mechanism is not novel vs v3 — this is a size-reduction exercise. Included
primarily as a parameter-efficiency ablation.
"""
...
class WeightSharedLM(AgiModel):
    """Size-reduced variant of FourierLinAttnLM via weight sharing + factorization.
    ...
    so the same weights behave differently at different depths.
    """

    version = "v4_weight_shared"
    architecture = "Weight-shared / factored linattn"
    cross_position = "Multi-head decay linattn (shared Q/K)"
    within_position = "Factored (diag + low-rank) channel mix"
