"""
v14_data_dependent: Extends v2_conv with input-dependent dynamics in the
style of Mamba / RWKV (data-dependent decay) and Hyena (input-modulated
convolution). Within-position channel mix uses a DCT basis instead of
sin/cos.

Hidden state is V-dimensional; input is one-hot; no output projection.

Per step:
  - Decay rate is a function of the current register state (rather than a
    fixed scalar), so tokens that should retain context get slow decay
    and tokens that should reset get fast decay. Same mechanism as
    Mamba's selective state.
  - Depthwise causal conv kernel weights are modulated by the current
    state at each position, producing an adaptive filter per token
    (Hyena-style).
  - Within-position mixer uses a DCT basis over vocab indices instead of
    the real+imag sin/cos basis used in v2; same caveat about vocab-id
    arbitrariness applies.

Untested on our benchmark; mechanisms borrowed from published work.
"""
...
class DataDepLM(AgiModel):
    """Data-dependent decay (Mamba), input-modulated conv (Hyena), DCT channel mix."""

    version = "v14_data_dependent"
    architecture = "Data-dependent dynamics"
    cross_position = "Input-modulated causal conv + data-dependent decay"
    within_position = "DCT-parameterized channel mix"
