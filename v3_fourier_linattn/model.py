"""
v3_fourier_linattn: Linear attention with exponential causal decay, where
Q/K/V/O projections are each parameterized as a linear combination of a
fixed Fourier basis over vocab indices.

Hidden state is V-dimensional; input is one-hot; no output projection.

Per step:
  attn_t = sum_{s < t} decay^(t-s-1) * (q_t . k_s) * v_s
  x = x + scale * FourierProjBack(attn_t)
  x = x + scale * FourierOp(rms_norm(x))

Q/K/V/O projections: each is a V x C matrix built as basis @ coeffs.T
(basis is a fixed V x 2K sin/cos grid over vocab indices; coeffs is learned
C x 2K). This constrains the projections to a 2K-dimensional subspace of
smooth functions over vocab-id ordering.

Caveat: vocab ids are arbitrary (BPE), so the "smoothness over vocab ids"
is a structural bottleneck rather than a useful prior. README confirms
this stalls training — val_loss 6.81, stuck.

The linear-attention-with-decay core is sound (same family as RWKV /
Linear Transformer); the Fourier parameterization is the failure mode.
"""
...
class FourierLinAttnLM(AgiModel):
    """Linear attention with causal decay; Q/K/V/O via fixed Fourier basis."""

    version = "v3_fourier_linattn"
    architecture = "Linear attention (Fourier parameterization)"
